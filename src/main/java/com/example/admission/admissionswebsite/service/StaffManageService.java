...
import com.example.admission.admissionswebsite.repository.StaffRepository;
...
public class StaffManageService {
    ...
    public UserDto getUserIdsByUsersRole() {
        UserDto resp = new UserDto();
        try {
            List<Users> users = staffRepository.findByRoles("STAFF");
            if (users.isEmpty()) {
                resp.setStatusCode(404);
                resp.setMessage("No users found with role 'STAFF'");
                return resp;
            }
